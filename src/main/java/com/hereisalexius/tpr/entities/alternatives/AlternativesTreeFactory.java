 ...
 */
public class AlternativesTreeFactory {

    public static AlternativesTree prepareTree(Class<? extends AlternativesTree> className, TransportationProblem tp) {
        AlternativesTree alternative = null;
        if (className.isAssignableFrom(ProbableTree.class)) {
            alternative = new ProbableTree(tp);
        } else {
            alternative = new CertainTree(tp);
        }
        return alternative;
    }
